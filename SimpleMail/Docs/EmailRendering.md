...
- Inline http(s) images and `background=`/`url()`/`srcset` URLs to data URIs with a local cache (up to ~16 MB total). If direct fetch fails, a generic public image proxy is used to match Gmail/Mail resilience.
